...
/**
 * Class CreateEventsIndexTest.
 */
class CreateEventsIndexTest extends TestCase
{
    /**
    ...
    public function testAsynchronous()
    {
        ...
        $command = new CreateEventsIndex(
            $repositoryReference,
            $token
        );

        $builtCommand = CreateEventsIndex::fromArray($command->toArray());
        $this->assertEquals(
            $command,
            $builtCommand
        );
