...
/**
 * Class DeleteEventsIndexTest.
 */
class DeleteEventsIndexTest extends TestCase
{
    /**
    ...
    public function testAsynchronous()
    {
        ...
        $command = new DeleteEventsIndex(
            $repositoryReference,
            $token
        );

        $builtCommand = DeleteEventsIndex::fromArray($command->toArray());
        $this->assertEquals(
            $command,
            $builtCommand
        );
